...
主要フロー
- サインイン: Email + Password、または OAuth(Apple/Google)。
...
画面構成（モバイル）
- Welcome（初回/任意）
  - CTA: 「ログイン」「新規登録」。規約/プライバシーリンク。
- サインイン
  - フィールド: Email, Password（表示/非表示切替）。
  - ボタン: 「ログイン」「パスワードをお忘れですか？」
  - OAuth: 「Appleで続行」「Googleで続行」。
- サインアップ
  - フィールド: Email, Password（強度の目安/ポリシー表示）。
  - 結果画面: 「確認メールを送信しました」＋再送/メール変更。
- パスワード再設定
  - フィールド: Email。
  - 結果画面: 「リセットメールを送信しました」（再送ガード）。
- 検証完了（Web→アプリ）
  - 表示: 「登録/受信許可を確認しました」→「アプリに戻る」。
- ログイン後初回
  - 受信者オンボーディングへ導線（受信者の追加・検証）。
...
導入順（提案）
1) サインイン/サインアップ/パスリセットの骨子 + Supabase連携。
2) VerifyPending/Deep Link整備、OAuth(Apple/Google)。
3) エラー文言/アクセシビリティ調整、計測。
4) ログイン後に受信者オンボーディングへ接続。
